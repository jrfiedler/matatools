{smcl}
{* 17may2014}{...}
{cmd:help mata mt_capture()}
{hline}

{title:Title}

{p 4 4 2}
{bf:mt_capture() {hline 2}} Capture function errors in Mata
...
{pstd}
{cmd:mt_capture()} runs the specified function with the specified arguments. If the
function aborts with error, {cmd:mt_capture()} returns the error code. If the
function does not abort with error, {cmd:mt_capture()} returns zero, and puts the
return value of the function in {it:rv_ptr}, if specified.

{pstd}
{cmd:mt_method_capture()} does the same for a class method.


{title:Examples}

{pstd}
{cmd:mt_capture()} is useful in the same situations as the Stata command 
...
it to. This can be done with {cmd:mt_capture()} and the built-in {cmd:assert()}.
Some tests you might write:

            assert( mt_capture(&thin(), &J(10, 1, 1)) == 0 )
            assert( mt_capture(&thin(), &J(0, 0, 1)) == 0 )
            assert( mt_capture(&thin(), &J(0, 10, 1)) == 0 )
            assert( mt_capture(&thin(), &J(10, 2, 1)) == 3200 )
            assert( mt_capture(&thin(), &J(10, 10, 1)) == 3200 )
